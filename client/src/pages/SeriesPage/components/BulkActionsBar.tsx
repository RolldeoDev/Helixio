...
export interface BulkActionsBarProps {
  ...
  /** Batch edit handler */
  onBatchEdit: () => void;
  /** Link series handler (optional, shown when 1-10 series selected) */
  onLinkSeries?: () => void;
}
  ...
  return (
    <div className="bulk-actions-bar" role="toolbar" aria-label="Bulk actions">
      ...
      <div className="bulk-actions-bar__actions">
        ...
        {/* Link Series - shown when 1-10 series selected */}
        {onLinkSeries && selectedCount >= 1 && selectedCount <= 10 && (
          <button
            className="bulk-actions-bar__action"
            onClick={onLinkSeries}
            disabled={isLoading}
            title="Link series to a parent"
          >
            <LinkIcon />
            <span>{selectedCount > 1 ? 'Bulk Link' : 'Link'}</span>
          </button>
        )}
