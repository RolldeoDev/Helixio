/**
 * IssueMetadataGeneratorPreviewModal Component
 *
 * Preview modal for LLM-generated issue metadata with confidence scores.
...
import type { GeneratedIssueMetadata } from '../../services/api.service';
import type { IssueMetadataGeneratorCurrentValues } from './IssueMetadataGenerator';
import { FieldComparisonRow } from '../FieldComparison';
import './IssueMetadataGeneratorPreviewModal.css';

// Field configuration
interface FieldConfig {
  key: keyof GeneratedIssueMetadata;
  label: string;
...
interface IssueMetadataGeneratorPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  onApply: (updates: Partial<IssueMetadataGeneratorCurrentValues>) => void;
  generatedMetadata: GeneratedIssueMetadata;
  currentValues: IssueMetadataGeneratorCurrentValues;
  issueName: string;
  webSearchUsed: boolean;
}

export function IssueMetadataGeneratorPreviewModal({
  isOpen,
  onClose,
  onApply,
  generatedMetadata,
  ...
  issueName,
  webSearchUsed,
}: IssueMetadataGeneratorPreviewModalProps) {
  // Track selected fields - default to fields that are empty
  const [selectedFields, setSelectedFields] = useState<Set<keyof GeneratedIssueMetadata>>(() => {
    const defaultSelected = new Set<keyof GeneratedIssueMetadata>();
  ...
  const toggleField = useCallback((fieldKey: string, selected: boolean) => {
    setSelectedFields((prev) => {
      ...
      if (selected) {
        next.add(fieldKey as keyof GeneratedIssueMetadata);
      } else {
        next.delete(fieldKey as keyof GeneratedIssueMetadata);
      }
      return next;
    });
  }, []);
  ...
  const selectAllEmpty = useCallback(() => {
    setSelectedFields(() => {
      const selected = new Set<keyof GeneratedIssueMetadata>();
      for (const config of FIELD_CONFIGS) {
        const currentValue = currentValues[config.key];
        const isEmpty = currentValue === null || currentValue === '' || currentValue === undefined;
        if (isEmpty) {
  ...
  const handleApply = useCallback(() => {
    const updates: Partial<IssueMetadataGeneratorCurrentValues> = {};
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="issue-metadata-generator-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-content">
          <div className="issue-info">
            <span className="issue-name">{issueName}</span>
            {webSearchUsed && (
