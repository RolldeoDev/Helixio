...
export async function createTemplate(input: CreateTemplateInput): Promise<TemplateWithParsedFields> {
  ...
  const template = await prisma.filenameTemplate.create({
    data: {
      ...
      folderSegments: input.folderSegments ? JSON.stringify(input.folderSegments) : null,
      characterRules: input.characterRules ? JSON.stringify(input.characterRules) : null,
